	$(document).ready(function() {
		...
		var std_table = $('#student_table').DataTable( {
			ajax: 'index.php?p=admin&c=Index&a=listStudents',
			columns: [
				{ data: "sid" },
				{ data: "sid",
	...
	function openStudentModal(sid,action)
	{
		...
		$.ajax({
			type: 'post',
			url: 'index.php?p=admin&c=Index&a=getStudentById',
			data: qs,
	...
	function addEditStudent(sid)
	{
		if($('#addEditStudentForm').validationEngine('validate')) {	
			...
			$.ajax({
				type: 'post',
				url: 'index.php?p=admin&c=Index&a=addEditStudent',
				data: qs,
	...
	function deleteStudent(sid,full_name){
		...
		if (conf == true) {
			...
			$.ajax({
				type: 'post',
				url: 'index.php?p=admin&c=Index&a=deleteStudentById',
				data: qs,
				success: function (res) {
					if(res)
					{
	...
	function logoutUser()
	{
		$.ajax({
			type: 'post',
			url: 'index.php?p=admin&c=Login&a=logoutUser',
			success: function (res) {
				if(res=="success")
				{
					window.location.replace("index.php?p=admin&c=Login&a=index");
				}
			},
			error: function(msg) {
				alert('Please reload the page!!!');
	...
	{
		window.open("index.php?p=admin&c=Index&a=exportCsv&type="+type);
	}